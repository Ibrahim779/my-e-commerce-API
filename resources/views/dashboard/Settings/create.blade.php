...
@endsection
@section('page_title', __('dashboard.settings').' | '.__('dashboard.add'))
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <form method="post" action="{{route('settings.store')}}" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                            <h5>{{__('dashboard.setting_add')}}</h5>
                            @if ($errors->any())
                            ...
                            <div class="form-group row">
                                <label for="key" class="col-sm-3 text-right control-label col-form-label">{{__('dashboard.key')}}</label>
                                <div class="col-sm-9">
                                    <input name="key" value="{{old('key')}}" type="text" class="form-control" id="key" placeholder="{{__('dashboard.key')}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-sm-3 text-right control-label col-form-label">{{__('dashboard.type')}}</label>
                                <div class="col-md-9">
                                    <select name="type" class="select2 form-control custom-select" style="width: 100%; height:36px;"><option  value="{{null}}">{{__('dashboard.select')}}</option>
                                        <option {{old('type') == 'text'?'selected':''}} value="text">{{__('dashboard.text')}}</option>
                                        <option {{old('type') == 'image'?'selected':''}} value="image">{{__('dashboard.image')}}</option>
                                        <option {{old('type') == 'textarea'?'selected':''}} value="textarea">{{__('dashboard.textarea')}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
